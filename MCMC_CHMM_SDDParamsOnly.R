...
for(g in 1:G){
  ...
  obs.mat[[g]] <- cbind(obs[1, 1, which(cstate.draws[1,1,2:200]==g)+1], obs[1, 1, which(cstate.draws[1,1,2:200]==g)])
  
  for(n in 2:N){
    obs.mat[[g]] <- rbind(obs.mat[[g]], 
                          cbind(obs[position[n,2], position[n,3], 
                                    which(cstate.draws[position[n,2], position[n,3],2:200]==g)+1], 
                                obs[position[n,2], position[n,3], 
                                    which(cstate.draws[position[n,2], position[n,3],2:200]==g)]))
  }
}
...
for(s in 2:500){
  ...
  # 
  # ## Sampling from tpm
  # covmat.candidate = matrix(c(0.3, 0, 0, 0.3), nrow=G-1, byrow=T)
  # 
  # for(g in 1:G){
  ...
  #                                          spatialparam.current = psi.draws[s-1], beta.row = g)
  ...
  #                                   position = position, neighbors = neighbors, 
  #                                   covmat.candidate = covmat.candidate, beta.current = beta.draws[s,,], 
  #                                   spatialparam.current = psi.draws[s-1])
