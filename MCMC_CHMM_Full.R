...
## Initialize MCMC
cstate.draws <- array(NA, c(5, 5, TT, S))
cstate.draws[,,,1] <- states
mu.draws[1,] <- mu
sigma.draws[1,] <- sigma
rho.draws[1,] <- rho
beta.draws[1,,] <- b0s
...
for(s in 2:S){
  ...
  for(k in index.reord){
    ...
    log_tpm_tr <- array(NA, c(G, G, TT-1))
    for(j in 1:(TT-1)){
  ...
  for(g in 1:G){
    ...
    obs.mat[[g]] <- cbind(obs[1, 1, which(cstate.draws[1,1,2:TT,s]==g)+1], obs[1, 1, which(cstate.draws[1,1,2:TT,s]==g)])
    
    for(n in 2:N){
      obs.mat[[g]] <- rbind(obs.mat[[g]], 
                            cbind(obs[position[n,2], position[n,3], 
                                      which(cstate.draws[position[n,2], position[n,3],2:TT,s]==g)+1], 
                                  obs[position[n,2], position[n,3], 
                                      which(cstate.draws[position[n,2], position[n,3],2:TT,s]==g)]))
    }
  }
  ...
  for(g in 1:G){
    beta.draws.acc <- beta.condpost.draw(N = N, TT = TT, G=G, state.draws = cstate.draws[,,,s], 
  ...
  psi.draws.acc <- psi.condpost.draw(N = N, TT = TT, G=G, state.draws = cstate.draws[,,,s],
                                     position = position, neighbors = neighbors,
